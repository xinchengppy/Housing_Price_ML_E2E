...
from google.cloud import storage       # GCP SDK for Cloud Storage
...
# Paths
# ----------------------------
# Downloads model + training features from GCS if not cached.
...
1. Imports (FastAPI, pandas, google.cloud.storage, your inference function).
2. Config setup (env vars → bucket/region).
3. GCS utility (load_from_gcs).
